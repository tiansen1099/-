...
export async function tableSearch(searchParam) {
  ...
  if (searchParam) {
    ...
    let searchUrl = `/di/DataCatalog/dcat/distributions/_table_search?diToken=${diToken}&start=${start}&length=${pageSize}`;
    if (orderCol) {
...
export async function tableSearchExport(searchParam) {
  const diToken = getSessionCache('diToken');
  if (searchParam) {
    const { currentPage, pageSize, orderCol, orderDir, conditions } = searchParam;
    const start = currentPage * pageSize;
    let exportUrl = `/di/DataCatalog/dcat/distributions/_table_search_export?diToken=${diToken}&start=${start}&length=${pageSize}`;
    
    if (orderCol) {
      exportUrl += `&order_col=${orderCol}`;
    }
    if (orderDir) {
      exportUrl += `&order_dir=${orderDir}`;
    }
...
export async function getDistributionDBSecretKey(payload) {
  const diToken = getSessionCache('diToken');
  return requestCustom(
    `/di/DataCatalog/dcat/acap/distributionReDevelopApi/getSecretKey?diToken=${diToken}`,
    {
      headers: {
        'Content-Type': 'application/json;charset=utf-8',
      },
 ...
 * @return {*}
 */
export async function checkPublish(payload) {
  const diToken = getSessionCache('diToken');
  const url = `/di/DataCatalog/dcat/acap/distributionReDevelopApi/checkDistributionCanPublish?diToken=${diToken}`;
  return request(url, {
    method: 'POST',
    body: payload,
  });
