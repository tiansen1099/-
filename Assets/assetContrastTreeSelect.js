...
class AssetContrastTreeSelect extends Component {
  constructor(props) {
    super(props);
    this.state = {
      value: '',
    ...
    if (res) {
      ...
      if (listData) {
        ...
        if (temp && temp.length !== 0) {
          treeData = this.convertElementsToTreeNodes(temp, 0);
        }
      }
    }
    this.setState({ treeData });
   ...
   * @returns {[]}
   */
  convertElementsToTreeNodes = (elements, pid) => {
    const treeData = [];
    if (elements && elements.length !== 0) {
      elements.forEach(element => {
        const treeNode = {};
        ...
        treeNode.value = element.id;
        const icon = element.icon.substring(element.icon.lastIndexOf('/') + 1);
        treeNode.title = element.namePath;
        treeNode.icon = (
          <img
            alt=""
            style={{ height: '18px', width: '18px', marginBottom: '3px' }}
  ...
  render() {
    const { treeData, expandedKeys, value } = this.state;
...
export default AssetContrastTreeSelect;
